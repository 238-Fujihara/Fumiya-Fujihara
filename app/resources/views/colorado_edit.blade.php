...
<body>
    ...
    <h1>Edmonds</h1>
    <form action="{{ route('coloradoPost.update',$coloradoposts->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('patch')
        ...
        @endif
            <div class="seatitle">
                <input type='text' name='title' value="{{ $coloradoposts['title'] }}">
            </div>
            <div class="seadate">
                <input type='date' name='date' value="{{ $coloradoposts['date'] }}">
            </div>
            <div class="seapics">
                <img src="{{ asset('/storage/images/' . $coloradoposts['image']) }}">
            </div>
        </div>
        <div class="ededit">
    ...
    <form action="{{ route('coloradoPost.destroy', $coloradoposts->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('delete')
